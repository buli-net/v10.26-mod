...
RUN --mount=target=/var/lib/apt/lists,type=cache,sharing=locked \
    ...
    apt-get update && \
    apt-get --yes --no-install-recommends install disorderfs openjdk-21-jdk-headless gradle sdkmanager && \
...
RUN --mount=target=/home/builder/android-sdk,type=cache,uid=1000,gid=1000,sharing=locked \
    ...
    gradle --project-dir project/ --no-build-cache --no-daemon --no-parallel clean :wallet:assembleRelease && \
    if [ -e /dev/fuse ]; then sleep 1 && fusermount -u project && rmdir project && mv project.u project; fi
